...
if (process.argv[2]) {
    ...
    const newKluddDirPath = path.join(process.cwd(), process.argv[2]);

    fs.mkdir(newKluddDirPath, { recursive: true }, (err) => {

        if (err) throw err;

        fs.copyFile(path.join(__dirname, 'template', 'index.html'), path.join(newKluddDirPath, 'index.html'), (err) => {
            if (err) throw err;
        });

        fs.copyFile(path.join(__dirname, 'template', 'script.js'), path.join(newKluddDirPath, 'script.js'), (err) => {
            if (err) throw err;
        });

        fs.copyFile(path.join(__dirname, 'template', 'style.css'), path.join(newKluddDirPath, 'style.css'), (err) => {
            if (err) throw err;
        });
    });

    process.chdir(path.join(newKluddDirPath));
    startServer();
